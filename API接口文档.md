...
```
1. 管理员登录 → POST /admin/login（获取Token）
2. 轮播图管理：
   - 查看列表 → GET /admin/banner/list
   - 添加轮播图 → POST /admin/banner/create
   - 编辑轮播图 → PUT /admin/banner/update
   - 删除轮播图 → DELETE /admin/banner/{id}
   - 启用/禁用 → PUT /admin/banner/status/{id}
3. 商户管理：
   - 查看列表 → GET /admin/merchant/list
   - 审核通过 → POST /admin/merchant/approve/{id}
   - 审核拒绝 → POST /admin/merchant/reject/{id}
   - 禁用商户 → POST /admin/merchant/disable/{id}
   - 启用商户 → POST /admin/merchant/enable/{id}
4. 用户管理：
   - 查看列表 → GET /admin/user/list
   - 禁用用户 → POST /admin/user/disable/{id}
   - 启用用户 → POST /admin/user/enable/{id}
5. 数据统计：
   - 概览数据 → GET /admin/statistics/overview
   - 趋势数据 → GET /admin/statistics/trend
```

### 12.2 商户端流程

```
1. 商户入驻 → POST /merchant/register
2. 商户登录 → POST /merchant/login（获取Token）
3. 创建门店 → POST /store/create
4. 发布盲盒 → POST /product/create
5. 查看商品 → GET /product/merchant/list
6. 下架商品 → POST /product/offline/{id}
7. 上架商品 → POST /product/online/{id}
8. 查看订单 → GET /order/merchant/list
9. 备餐完成 → POST /order/ready/{orderNo}
10. 核销订单 → POST /order/verify/{pickupCode}
11. 同意退款 → POST /order/refund/approve/{orderNo}
12. 拒绝退款 → POST /order/refund/reject/{orderNo}
```

### 9.3 用户端流程

```
1. 微信登录 → POST /user/login/wechat（获取Token）
2. 获取用户信息 → GET /user/info
3. LBS发现周边门店 → GET /store/nearby（传入用户位置）
4. 查看门店详情 → GET /store/explore/{id}
5. 浏览商品 → GET /product/list
6. 查看商品详情 → GET /product/detail/{id}
7. 点击"立即购买" → 弹出购买确认弹窗（淘宝风格底部弹窗）
8. 点击"立即支付" → 弹出支付方式选择弹窗
9. 选择支付方式 → 创建订单 → POST /order/create
10. 完成支付 → POST /order/pay/{orderNo}
    - 支持微信支付（暂为模拟）
    - 支持模拟支付（直接完成支付）
    - 订单创建后15分钟内需完成支付，否则自动取消
11. 查看我的订单 → GET /order/my
12. 查看订单详情 → GET /order/detail/{orderNo}
13. 取消订单 → POST /order/cancel/{orderNo}（仅待支付可取消）
14. 申请退款 → POST /order/refund/{orderNo}（已支付/待取餐可申请，提交后等待商户审核）
```
...
**v1.17 (2026-03-06)**
- 新增第十章：即时通讯接口
  - WebSocket连接信息：`GET /im/ws/info`
  - 会话管理：创建会话、获取会话列表、获取会话详情、结束会话
  - 消息管理：获取消息列表、发送消息、撤回消息
  - 平台端专用接口：`POST /im/session/platform/user`、`POST /im/session/platform/merchant`
- 新增IM消息管理权限
  - `im` - 消息管理菜单
  - `im:session` - 会话列表权限
  - `im:session:view` - 查看会话权限
  - `im:session:end` - 结束会话权限
  - `im:message` - 消息记录权限
  - `im:message:view` - 查看消息权限
  - `im:message:send` - 发送消息权限
- 客服角色新增消息管理权限

**v1.16 (2026-03-05)**
- 新增第八章：平台管理权限接口
  - 角色管理接口：`GET /admin/role/list`、`POST /admin/role/create`、`PUT /admin/role/update`、`DELETE /admin/role/{id}`、`POST /admin/role/assign`
  - 权限管理接口：`GET /admin/permission/tree`（按 parent_code 构建权限树）
  - 账号管理接口：`GET /admin/account/list`、`POST /admin/account/create`、`PUT /admin/account/update`、`DELETE /admin/account/{id}`、`POST /admin/account/resetPwd`、`PUT /admin/account/password`
  - 操作日志接口：`GET /admin/log/list`（支持多维度筛选）
- 新增超级管理员通配符权限 `*:*`，登录时自动展开所有具体权限

**v1.15 (2026-03-04)**
- 新增商户统计接口 `GET /merchant/stats`（第二章 2.4节）
- 新增商户营收趋势接口 `GET /merchant/revenue/trend`（第二章 2.5节）
- 新增商户历史营收接口 `GET /merchant/revenue/history`（第二章 2.6节）

**v1.14 (2026-03-04)**
- 更新用户订单列表接口 `GET /order/my`（第六章 6.3节）
  - 新增状态筛选参数 `status`（支持全部、待支付、待取货、已完成、已取消、退款中）
  - 新增订单号搜索参数 `orderNo`（模糊匹配）
  - 新增分页参数 `page`、`size`
  - 返回格式改为分页格式 `{list, total, page, size}`
- 新增 `status=-2` 待取货筛选（包含已支付+待取餐状态）

**v1.11 (2026-03-04)**
- 新增用户统计数据接口 `GET /user/stats`（第五章 5.4节）
  - 订单统计：待支付、待取货、已完成、已取消、退款中数量
  - 金额统计：累计消费、累计节省、本月消费、本月节省
  - 环保成就：拯救食物重量、减碳量、环保行动次数
  - 余额相关：账户余额、累计充值金额
- 完善个人主页数据看板功能

**v1.10 (2026-03-04)**
- 新增余额管理接口（第五章 5.5、5.6节）
  - 余额充值接口 `POST /balance/recharge`
  - 余额变动记录接口 `GET /balance/logs`
- 新增余额变动类型说明（充值、消费、退款、系统赠送）
- 完善虚拟余额管理功能

**v1.9 (2026-03-04)**
- 新增用户端支付流程说明（淘宝风格底部弹窗、支付方式选择）
- 更新用户端业务流程，添加支付方式选择步骤
- 优化库存并发控制日志记录，保证高并发下日志准确性
- 使用 `@Transactional` 保证 UPDATE 和 SELECT 在同一事务中执行
- 行锁持续到事务提交，确保日志记录的 before/after 值准确

**v1.8 (2026-03-04)**
- 新增库存并发控制说明（预占机制、库存操作流程、定时任务）
- 新增库存表字段说明（total_stock, available_stock, locked_stock）
- 完善并发安全保障说明

**v1.7 (2026-03-04)**
- 新增取餐码二维码格式说明
- 更新核销接口说明，支持多种核销方式（手动输入/摄像头扫码/图片识别）
- 完善订单详情接口说明

**v1.6 (2026-03-03)**
- 新增门店接口（第三章）
  - 周边门店列表接口 `GET /store/nearby`（用户端LBS发现）
  - 门店详情接口 `GET /store/explore/{id}`（用户端探索页）
  - 更新门店接口 `PUT /store/update`
- 更新用户端业务流程，添加LBS发现功能
...
**v1.4 (2026-03-02)**
- 新增24小时自动退款功能（商户超时未处理自动批准退款）
- 新增订单字段 `refundApplyTime`（退款申请时间）
- 更新状态流转图（增加超时自动退款路径）
- 更新申请退款接口说明

**v1.3 (2026-03-02)**
- 新增退款审核流程（状态6-退款中）
- 新增商户同意退款接口 `POST /order/refund/approve/{orderNo}`
- 新增商户拒绝退款接口 `POST /order/refund/reject/{orderNo}`
- 新增商户订单列表接口 `GET /order/merchant/list`
- 更新申请退款接口说明（状态变为退款中，需商户审核）
- 更新订单状态流转图
